        ...
        if (ValidateForm()) {
            try {
                ...
                if (error.response && error.response.data.errors) {
                    if (error.response.data.errors.UserExist) {
                        setErrors(prev => ({ ...prev, username: "User already exists" }));
                    }
                } else {
        ...
        <div className='bg-white rounded-2xl WidthOfForm p-6 mx-auto mt-20 mb-20'>
            ...
            <div className='mt-4 text-center'>
                <p>Already have an account?&nbsp; 
                    <Link to="/login" className='font-bold hover:font-thin duration-300 hover:opacity-50 cursor-pointer'>Log In</Link>
                </p>
            </div>
        </div>
    );
